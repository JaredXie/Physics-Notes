# Physics-Notes

My intention in building this repository is to give a glance at the structure of physics, grounded in my understanding and learning pathway. It's been a long way since I studied education and science at the same time, across a wide endeavour and broad discipline. 

Adapted and detailing my thoughts from [“脑子里的物理学体系” by 哈嘞卤吖 on Zhihu](https://zhuanlan.zhihu.com/p/600949458), here I wrote the structure of physics during my learning pathway. It seems structured, though, not content or satisfactory for many reasons. But I would still use it as my best structure out of my effort so far.

# Physics Knowledge Map

This page organises Physics knowledge from secondary school to university level.  
It helps me understand how school Physics connects with more advanced Physics learning, and how different topics can be developed into teaching resources.

---

## 1. Junior Secondary Physics

At the junior secondary level, Physics is usually introduced through observable phenomena and everyday experiences.  
The focus is on building basic concepts, scientific vocabulary, and simple models.

### Main Areas

- **Forces and Motion**
  - Basic forces
  - Motion
  - Friction
  - Gravity
  - Simple machines
  - Basic sound concepts

- **Light**
  - Reflection
  - Refraction
  - Shadows
  - Lenses
  - Colour

- **Heat**
  - Temperature
  - Heat transfer
  - Conduction
  - Convection
  - Radiation
  - Changes of state

- **Electricity and Magnetism**
  - Simple circuits
  - Current
  - Voltage
  - Resistance
  - Series and parallel circuits
  - Magnets
  - Electromagnets

---

## 2. Senior Secondary Physics

At the senior secondary level, Physics becomes more systematic and mathematical.  
Students begin to connect physical concepts with equations, models, experiments, and real-world applications.

### Main Areas

- **Mechanics**
  - Kinematics
  - Dynamics
  - Newton's laws
  - Momentum
  - Work, energy and power
  - Circular motion
  - Gravitation

- **Electricity and Magnetism**
  - Electric fields
  - Magnetic fields
  - Electric circuits
  - Electromagnetic induction
  - Motors and generators

- **Atomic and Nuclear Physics**
  - Atomic structure
  - Radioactivity
  - Half-life
  - Nuclear fission
  - Nuclear fusion
  - Applications of nuclear Physics

- **Light and Waves**
  - Wave properties
  - Reflection
  - Refraction
  - Diffraction
  - Interference
  - Wave-particle ideas

- **Thermal Physics**
  - Heat transfer
  - Internal energy
  - Specific heat capacity
  - Latent heat
  - Gas laws
  - Thermal equilibrium

---

## 3. University Physics

University Physics develops from general Physics into theoretical Physics, experimental Physics, and technical research methods.

---

## 3.1 Theoretical Knowledge

### 3.1.1 General Physics

General Physics provides the foundation for more advanced study.

- **Mechanics**
  - Classical mechanics
  - Oscillations
  - Waves
  - Introductory special relativity

- [**Thermal Physics**](./Thermodynamics_n_Statistics.md)
  - Temperature
  - Heat
  - Thermodynamics
  - Kinetic theory

- **Electricity and Magnetism**
  - Electrostatics
  - Magnetostatics
  - Electromagnetic induction
  - Circuits
  - Basic electrical and electronic engineering

- **Optics**
  - Geometrical optics
  - [Physical optics](./Optics_Electromagnetism.md)
  - [Interference](./Optics_Interference.md)
  - Diffraction
  - Polarisation

- **Atomic Physics**
  - Atomic structure
  - Spectra
  - Quantum ideas
  - Nuclear phenomena

---

### 3.1.2 Core Theoretical Physics

The core theoretical Physics courses are often known as the four major mechanics.

- **Theoretical Mechanics / Analytical Mechanics**
  - Lagrangian mechanics
  - Hamiltonian mechanics
  - Conservation laws
  - Variational principles

- [**Electrodynamics**](./Electrodynamics.md)
  - Maxwell's equations
  - Electromagnetic waves
  - Radiation
  - Relativistic electrodynamics

- [**Quantum Mechanics**](./Quantum_Mechanics.md)
  - [Wave functions](./Wave_functions)
  - Schrödinger equation
  - Operators
  - Angular momentum
  - Perturbation theory

- [**Thermodynamics and Statistical Physics**](./Thermodynamics_n_Statistics.md)
  - Laws of thermodynamics
  - Entropy
  - Ensembles
  - Statistical distributions
  - Phase transitions

### Supplementary Courses

- Laser Principles
- Physical Optics
- [Semiconductor Physics](./Semiconductor_Physics.md)

---

### 3.1.3 Advanced Theoretical Physics

Advanced Physics courses focus on specialised and research-oriented areas.

- Solid State Physics
- Advanced Quantum Mechanics
- Quantum Field Theory
- Quantum Statistics
- Particle Physics
- Relativity
- Quantum Optics
- Introduction to Condensed Matter Physics
- Nonlinear Optics
- Group Theory
- Lie Groups
- Other specialised topics

---

## 3.2 Experimental Physics

Experimental Physics develops practical research skills, measurement techniques, data analysis, and the ability to connect theory with evidence.

### Main Experimental Areas

- General Physics Experiments
- Modern Physics Experiments
- Electrical and Electronic Engineering Experiments
- Thin Film Physics and Technology
- Specialised Experimental Physics Courses

---

## 3.3 Technical Tools and Research Skills

Physics learning and research also require technical tools for data processing, simulation, academic writing, and numerical modelling.

### Common Tools

- **Origin**
  - Data analysis
  - Graphing
  - Curve fitting

- **EndNote**
  - Reference management
  - Academic citation organisation

- **Microsoft Office**
  - Reports
  - Presentations
  - Tables and documentation

- **GeoGebra**
  - Mathematical visualisation
  - Graphs
  - Geometry and functions

- **Python**
  - Data analysis
  - Numerical calculation
  - Visualisation
  - Simulation

- **C / C++**
  - Scientific computing
  - Numerical methods
  - Performance-based programming

- **MATLAB**
  - Numerical calculation
  - Matrix operations
  - Engineering and Physics modelling

- **Mathematica**
  - Symbolic computation
  - Analytical calculation
  - Mathematical modelling

- **Materials Studio**
  - Materials simulation
  - Molecular modelling

- **COMSOL**
  - Multiphysics simulation
  - Finite element modelling

- **VASP**
  - First-principles calculation
  - Density functional theory

- **Gaussian**
  - Computational chemistry
  - Molecular orbital calculation
  - Quantum chemistry simulation

---

## 4. Reflection for Secondary Physics Teaching

This knowledge map reminds me that school Physics is the beginning of a much larger Physics system.

For secondary teaching, I do not need to teach university-level Physics directly. However, understanding the broader structure helps me:

- explain school Physics more clearly;
- identify how simple concepts develop into advanced theories;
- avoid teaching isolated facts;
- connect classroom activities with real scientific thinking;
- prepare students for further study in Physics, engineering, and related fields.

The key teaching challenge is to make Physics accessible without oversimplifying it.  
A good Physics teacher should help students move from everyday observations to scientific models, and from simple explanations to deeper reasoning.
